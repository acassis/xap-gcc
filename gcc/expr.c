...
struct { rtx pseudo; int reg; int n_regs; } pending_reg_arg;
     ...
     rtx alignment_pad;
{
  ...
  if (partial > 0 && reg != 0)
	...
	emit_group_load (reg, x, -1);  /* ??? size? */
      ...
      {
        /* pending_reg_arg is guarenteed to be initialised to all-bits-zero,
         * not NULL. Ha! Like I care. */
        rtx pseudo = gen_reg_rtx(GET_MODE(reg));
        if(pending_reg_arg.pseudo != NULL) abort();
        move_block_to_reg (REGNO (reg), x, partial, mode);
        emit_move_insn (pseudo, reg);
        pending_reg_arg.pseudo = pseudo;
        pending_reg_arg.reg = REGNO(reg);
        pending_reg_arg.n_regs = partial;
      }
    }
...

void resolve_pending_reg_arg()
{
    if(pending_reg_arg.pseudo)
    {
        move_block_to_reg(pending_reg_arg.reg,
                          pending_reg_arg.pseudo,
                          pending_reg_arg.n_regs,
                          GET_MODE(pending_reg_arg.pseudo));

        pending_reg_arg.pseudo = NULL;
